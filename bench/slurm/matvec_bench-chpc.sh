...
for NP in 1 16 64 ; do
  mpirun -np $NP ./$RUNPROGRAM $(($PTS_PER_PROC / $NP)) $MAX_DEPTH $ELE_ORDER "strong";
  echo ;
done
...
ELE_ORDER=1
for NP in 1 16 64 ; do
  mpirun -np $NP ./$RUNPROGRAM $PTS_PER_PROC $MAX_DEPTH $ELE_ORDER "weak";
  echo ;
done
