...
// ========================================================================== //
MPI_TEST_CASE("Nonuniform Poisson gmg sinusoid", 1)
{
  MPI_Comm comm = test_comm;
  DendroScopeBegin();
  constexpr int dim = 2;
  ...
  if(rank == 0)
  {
    std::cout << "___________Begin poissonEq GMG ("
  ...
  if(!rank)
  {
    std::cout << "___________End of poissonEq GMG: "
              << "Finished "
              << "in " << steps << " iterations. "
              << "Final error==" << err << "\n";
  }
...
// ========================================================================== //
MPI_TEST_CASE("Nonuniform Poisson hybrid sinusoid", 1)
{
  MPI_Comm comm = test_comm;
  DendroScopeBegin();
  constexpr int dim = 2;
  ...
  // Linear system oracles encapsulating the operators.
  PoissonEq::HybridPoissonMat<dim> &base_mat = *fe_matrix(base_da);  //future: not a pointer
  PoissonEq::PoissonVec<dim> base_vec = fe_vector(base_da);
  ...
  std::vector<HybridPoissonMat *> mats(n_grids, nullptr);
  mats[0] = &base_mat;
  for (int g = 1; g < n_grids; ++g)
  {
    mats[g] = fe_matrix(*das[g].primary);
  ...
  if(rank == 0)
  {
    std::cout << "___________Begin poissonEq Hybrid ("
              << par::mpi_comm_size(comm)
              << " processes)\n";
  }
  ...
  if(!rank)
  {
    std::cout << "___________End of poissonEq Hybrid: "
              << "Finished "
              << "in " << steps << " iterations. "
              << "Final error==" << err << "\n";
  }
