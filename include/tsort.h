...
namespace type
{
  ...
  struct StrongIndex
  {
    ...
    StrongIndex &operator+=(const T d)
    {
      m_i += d;
      return *this;
    }
...
namespace ot
{
  ...
  {
    const TreeNode<T, dim> operator()(TreeNode<T, dim> tn)
    {
      tn.setLevel(m_uiMaxDepth);
      return tn;
    }
  };
  ...
  struct SFC_Tree
  {
    ...
    static void locTreeSortMaxDepth(std::vector<PointType> &points)
    {
      SFC_Tree<T, dim>::locTreeSort<KeyFunIdentity_maxDepth<T, dim>,
                                    PointType, TreeNode<T, dim>, false, int>(&(*points.begin()), 0, (RankI)points.size(), 0, m_uiMaxDepth, SFC_State<dim>::root(),
    ...
    {
      SFC_Tree<T, dim>::locTreeSort<KeyFunIdentity_maxDepth<T, dim>,
                                    PointType, TreeNode<T, dim>, true, CompanionT...>(&(*points.begin()), 0, (RankI)points.size(), 0, m_uiMaxDepth, SFC_State<dim>::root(),
