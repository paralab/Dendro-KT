...
namespace ot
{
     ...
     namespace DA_FLAGS
     {
          ...
          enum LoopType
          {
               ...
               W_DEPENDENT,
               W_BOUNDARY
          }; ///,LOCAL_ELEMENTS,PREGHOST_ELEMENTS,POSTGHOST_ELEMENTS};
          ...
          enum Refine
          {
               DA_NO_CHANGE = OCT_FLAGS::OCT_NO_CHANGE,
     ...
     public:
          ...
          DA(const DistTree<C, dim>&&inDistTree, MPI_Comm comm, unsigned int order, size_t grainSz = 100, double sfc_tol = 0.3) = delete;
          ...
          DA(const DistTree<C, dim>&&inDistTree, int stratum, MPI_Comm comm, unsigned int order, size_t grainSz /*= 100*/, double sfc_tol /*= 0.3*/, int version = 0) = delete;
